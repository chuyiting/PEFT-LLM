...
class MisconceptionDataset(Dataset):
    def __init__(self, model, tokenizer, k, data_path, cluster_path, misconception_map_path, device):
    ...
    def generate_prompt(self, row):
        prompt = self.preprocess_text(f"""Here is a question about {row['ConstructName']} ({row['SubjectName']}):
        
        - Question: {row['QuestionText']}
        - Correct Answer: {row['CorrectAnswerText']}
        - Wrong Answer: {row['WrongAnswerText']}
        
        Please identify the likely misconception or reasoning error that led the student to choose the wrong answer. Focus only on explaining the error.""")
    ...
    def apply_anchor_prompt(self, data):
    ...
    def get_topK_negatives(self, data):
        self.model.eval()
...
if __name__ == "__main__":
    ...
    # Load dataset
    dataset = MisconceptionDataset(model, tokenizer, k=args.k, data_path=data_path, cluster_path=cluster_path, misconception_map_path=misconception_map_path, device=device)
