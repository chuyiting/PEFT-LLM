    ...
    for epoch in tqdm(range(epochs), desc="Epochs"):
        ...
        for batch in tqdm(dataloader):
            ...
            if not use_unsloth:
                ...
                new_negative_embeddings = generate_new_negatives(negative_hidden_states, 2)
                new_positive_embeddings = generate_new_positive(prompt_hidden_state, positive_hidden_state, negative_hidden_states[:, 0, :])
            else:
                #with autocast(device_type='cuda'):
            ...
            synthetic_loss = loss_fn(prompt_hidden_state, new_positive_embeddings, new_negative_embeddings)
            origin_loss = loss_fn(prompt_hidden_state, positive_hidden_state, negative_hidden_states)
            loss = origin_loss + (0.5 * synthetic_loss)
            loss.backward()
